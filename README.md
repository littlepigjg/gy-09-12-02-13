...
- **共同好友**：邻接集合求交集，并按关系强度/朋友圈重叠/同社群三路信号排序，逐名给出解释
...
| GET  | `/api/common_friends?node1&node2` | 共同好友（按亲疏远近排序 + 逐名解释，状态严格区分） |
...
- **共同好友排序**：在完整保留交集名单的前提下，按三路可解释信号对每个共同好友评分排序——
  45% 平均关系强度（与双方的边权按各自最高边权归一化）+ 40% 平均朋友圈重叠度（邻居 Jaccard）
  + 15% 同社群（Louvain）加分；总分相同按人脉广度（本人好友数）决胜。
  每位好友都返回：名次、综合分、一句话关系定性、逐条可核对的量化理由、与上一名的差距说明。
  状态严格区分，绝不混淆：`node_missing`（节点不存在，逐个点名 missing_nodes）、`same_node`（两人是同一节点）、
  `no_common`（都存在但没有共同好友）、`ok`（正常返回，`ranked` 与平铺 `friends` 集合完全一致）。
  四种状态均返回 HTTP 200，靠 body 中的 `status` 字段判别——「查无此人」是一种查询答案而非请求错误，
  避免前端把不存在和没共同好友都吞成笼统的「请求失败」。
